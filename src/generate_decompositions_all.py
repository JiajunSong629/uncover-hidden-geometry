...
for i in tqdm(range(n_batch), desc="Get data for pos and global mean"):
    ...
    with torch.no_grad():
        hiddens = model.generate_hiddens_all(x)  # (L, 6, B, T, C)
        if i == 0:
            embeddings_mean_by_B = (hiddens / (n_batch * batch_size)).sum(2)
            ...
            embeddings_mean_by_T = np.array(
                [
                    np.concatenate([new, old], axis=1)  # L, 6, B, C
                    for old, new in zip(
                        embeddings_mean_by_T,
                        (
                            hiddens[:, :, :, id_start:id_end, :] / (id_end - id_start)
        ...
        for i, token in enumerate(special_vocab):
            ...
            if token not in token_embd:
                token_embd[token] = hiddens[:, :, is_x, :].sum(2)
...
L, types, T, C = embeddings_mean_by_B.shape
L, types, B, C = embeddings_mean_by_T.shape
Tp = id_end - id_start
...
for layer_idx in tqdm(range(L), desc="Calculate pos and global mean"):
    ...
    global_mean = np.mean(mean_vec_by_B, axis=1, keepdims=True)
...
for i in tqdm(range(n_cvec_batch), desc="Get data for cvec and resid"):
    x, y = get_batch(split, batch_size)
    with torch.no_grad():
        hiddens = model.generate_hiddens_all(x)

        for layer_idx in range(L):
            for type_idx in range(types):
                embeddings[
                    i * batch_size : i * batch_size + batch_size, layer_idx, type_idx
                ] = hiddens[layer_idx, type_idx]
